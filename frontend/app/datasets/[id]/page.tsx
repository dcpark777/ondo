...
export default function DatasetDetailPage() {
  ...
  const validTabs: Array<'overview' | 'score' | 'schema' | 'lineage' | 'quality' | 'usage' | 'activity' | 'details'> = ['overview', 'score', 'schema', 'lineage', 'quality', 'usage', 'activity', 'details']
  
  // Get initial tab from URL
  const getTabFromUrl = (): 'overview' | 'score' | 'schema' | 'lineage' | 'quality' | 'usage' | 'activity' | 'details' => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search)
      const tab = params.get('tab')
      if (tab && validTabs.includes(tab as any)) {
        return tab as 'overview' | 'score' | 'schema' | 'lineage' | 'quality' | 'usage' | 'activity' | 'details'
      }
    }
    return 'overview'
  }

  const [activeTab, setActiveTab] = useState<'overview' | 'score' | 'schema' | 'lineage' | 'quality' | 'usage' | 'activity' | 'details'>(getTabFromUrl())

  // Update URL when tab changes
  const handleTabChange = (tab: 'overview' | 'score' | 'schema' | 'lineage' | 'quality' | 'usage' | 'activity' | 'details') => {
    setActiveTab(tab)
    const params = new URLSearchParams(window.location.search)
    if (tab === 'overview') {
